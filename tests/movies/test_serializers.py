...
def test_valid_movie_serializer():
    valid_serialiazer_data = {
        "title": "Raising Arizona",
        "genre": "comedy",
        "year": "1987",
    }
    serializer = MovieSerializer(data=valid_serialiazer_data)
    assert serializer.is_valid()
    assert serializer.validated_data == valid_serialiazer_data
    assert serializer.data == valid_serialiazer_data
